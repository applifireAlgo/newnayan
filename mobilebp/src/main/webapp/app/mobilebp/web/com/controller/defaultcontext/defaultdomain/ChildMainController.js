Ext.define('Mobilebp.mobilebp.web.com.controller.defaultcontext.defaultdomain.ChildMainController', {
     extend: 'Mobilebp.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.ChildMainController',
     init: function() {
          var form = this.view.down('#ChildForm');
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#ChildTree');
               ...
               var compRef = this.view.down('#ChildTree');
     ...
     resetForm: function(but) {
          ...
          form.down('#saveFormButton').setText('Save');
          form.reset();
          var grid = this.view.down('#ChildGrid');
          var tree = this.view.down('#ChildTree');
          tree.setSelection();
          grid.setSelection();
     },
     hideDataBrowser: function() {
          var form = this.view.down('#Child');
          var grid = this.view.down('#ChildGrid');
          var tree = this.view.down('#ChildTreeContainer');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ChildForm');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#ChildTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          ...
          if (record.data.leaf) {
               var Child = this.view.down('#ChildForm');
               Child.reset();
               var gridPanel = this.view.down('#ChildGrid');
          ...
          var formPanel = this.getView().up().down('#ChildForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ChildTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Child').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ChildGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
