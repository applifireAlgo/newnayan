Ext.define('Mobilebp.mobilebp.web.com.controller.defaultcontext.defaultdomain.TestUserMainController', {
     extend: 'Mobilebp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestUserMainController',
     ...
     newuserLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#newuser');
                    scope.sender.controller.assignAllComboData(scope.sender, 'newuser', response.responseText);
     ...
     fNameLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#fName');
                    scope.sender.controller.assignAllComboData(scope.sender, 'fName', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#fName');
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestUserTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestUserForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestUserGrid');
               var tree = this.view.down('#TestUserTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestUserForm');
          var grid = this.view.down('#TestUserGrid');
          var tree = this.view.down('#TestUserTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestUserGrid');
               ...
               var formPanel = this.getView().up().down('#TestUserForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestUserTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestUserForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestUserGrid').store;
          gridStore.removeAll();
