Ext.define('Mobilebp.mobilebp.web.com.controller.defaultcontext.defaultdomain.TestEMainController', {
     extend: 'Mobilebp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestEMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestETree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestEForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestEGrid');
               var tree = this.view.down('#TestETree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestEForm');
          var grid = this.view.down('#TestEGrid');
          var tree = this.view.down('#TestETreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestEGrid');
               ...
               var formPanel = this.getView().up().down('#TestEForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestETree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestEForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestEGrid').store;
          gridStore.removeAll();
