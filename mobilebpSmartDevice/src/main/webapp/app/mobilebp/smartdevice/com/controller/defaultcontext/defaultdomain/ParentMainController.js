Ext.define('Mobilebp.mobilebp.smartdevice.com.controller.defaultcontext.defaultdomain.ParentMainController', {
     extend: 'Mobilebp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ParentMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ParentTree');
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Parent');
          var grid = this.view.down('#ParentGrid');
          var tree = this.view.down('#ParentTreeContainer');
          grid.setHidden(true);
     ...
     renderTreeGridData: function() {
          ...
          if (responseData.response.success) {
               ...
               tree.getStore().sort('text', 'ASC');
               var compRef = this.view.down('#ParentTree');
          } else {
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#ParentTree');
          treePanel.collapse();
          var formPanel = this.getView().up().down('#Parent');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Parent').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ParentGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#Parent').getViewModel().setData(responseData.response.data);
     },
