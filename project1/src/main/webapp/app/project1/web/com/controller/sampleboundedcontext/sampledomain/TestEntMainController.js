Ext.define('Project1.project1.web.com.controller.sampleboundedcontext.sampledomain.TestEntMainController', {
     extend: 'Project1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestEntMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestEntTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestEntForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestEntGrid');
               var tree = this.view.down('#TestEntTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestEntForm');
          var grid = this.view.down('#TestEntGrid');
          var tree = this.view.down('#TestEntTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestEntGrid');
               ...
               var formPanel = this.getView().up().down('#TestEntForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestEntTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestEntForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestEntGrid').store;
          gridStore.removeAll();
