...
@Repository
@SourceCodeAuthorClass(createdBy = "nayan.chaudhari@algorhythm.co.in", updatedBy = "", versionNumber = "1", comments = "Repository for TestA Transaction table", complexity = Complexity.MEDIUM)
public class TestARepositoryImpl extends SearchInterfaceImpl implements TestARepository<TestA> {
    ...
    @Transactional
    public TestA findById(String testaid) throws Exception {
        javax.persistence.EntityManager emanager = emfResource.getResource();
        javax.persistence.Query query = emanager.createNamedQuery("TestA.findById");
        query.setParameter("testaid", testaid);
